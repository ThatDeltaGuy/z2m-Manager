...
def _options_schema_dict(defaults: dict[str, Any]) -> dict[Any, Any]:
    """Build the tunable-options form fields.

    Shared by the initial "add integration" step and the options flow, so
    the same settings are available immediately on setup (not just
    afterwards via Settings > ... > Configure) and the two forms can never
    drift out of sync.
    """
    return {
        vol.Optional(
        ...
        vol.Optional(
            CONF_NETWORKMAP_TYPE,
            default=defaults.get(CONF_NETWORKMAP_TYPE, DEFAULT_NETWORKMAP_TYPE),
...
class Z2MManagerConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        """Collect the instance name/base topic, plus every tunable option up front."""
        ...
        if user_input is not None:
            ...
            if not base_topic:
                ...
                options = {
                    key: value for key, value in user_input.items() if key not in (CONF_NAME, CONF_BASE_TOPIC)
                }
...
class Z2MManagerOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        return self.async_show_form(
            step_id="init",
            data_schema=vol.Schema(_options_schema_dict(self.config_entry.options)),
        )
